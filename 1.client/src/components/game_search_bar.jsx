...
function GameSearchBar({ onSearchChanged, searchResults }) {
  ...
  return (
    ...
    <motion.div layout className="flex flex-col w-full bg-base-200 shadow-md rounded-xl p-4" style={{ height: resultsOpen ? "300px" : "" }}>
      ...
      <div>{resultsOpen && <GameSearchDropdown results={searchResults} />}</div>
    </motion.div>
  );
}
