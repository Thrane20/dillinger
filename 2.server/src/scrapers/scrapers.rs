...
impl Scraper {
    ...
    pub async fn get_matching_titles(name: String, gamedb : Option<&mut Box<dyn GameDatabase>>) -> Option<Vec<ScrapeEntry>> { 
        
        if let Some(db) = gamedb {
            // Search the games database for any specific matching titles
            let scraped_entries = &db.search_game(name.as_str()).await;
    ...
    pub async fn get_matching_platforms(name: String, gamedb : Option<&mut Box<dyn GameDatabase>>) -> Option<Vec<PlatformEntry>> { 
        
        if let Some(db) = gamedb {
            // Search the games database for any specific matching platforms
            let scraped_entries = &db.search_platform(name.as_str()).await;
            return Some(scraped_entries.to_vec());
        } else {
