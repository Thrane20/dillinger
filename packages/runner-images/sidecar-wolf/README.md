...
A dedicated streaming sidecar that provides a single "Dillinger" app visible in Moonlight. Game runner containers connect to this sidecar's Wayland compositor, and their output is streamed.

## Overview

This sidecar separates the streaming infrastructure from individual game runners:

- **Single App**: Moonlight sees one app called "Dillinger" - whatever game is running appears there
- **Ubuntu-based**: Uses Ubuntu 24.04 with Wolf binary for GStreamer compatibility
- **Hardware Encoding**: VA-API (AMD/Intel) or NVENC (NVIDIA) with x264 fallback
...
docker run -d \
  ...
  -v dillinger_streaming_wayland:/run/dillinger \
  --device /dev/dri \
  -e GPU_TYPE=amd \
  -e SIDECAR_MODE=game \
  -e IDLE_TIMEOUT_MINUTES=15 \
  ghcr.io/thrane20/dillinger/streaming-sidecar:0.4.0
```
...
docker run -it \
  ...
  ghcr.io/thrane20/dillinger/runner-wine:0.4.0
```
