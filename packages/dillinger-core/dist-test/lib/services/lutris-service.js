...
exports.searchLutrisInstallers = searchLutrisInstallers;
exports.getLutrisInstaller = getLutrisInstaller;
exports.parseLutrisScript = parseLutrisScript;
exports.getInstallerFileKey = getInstallerFileKey;
 ...
 */
async function searchLutrisWithVariations(searchQueries, gogId) {
    const gogIdStr = String(gogId);
    for (const query of searchQueries) {
        console.log(`[Lutris] Trying search: "${query}"`);
        try {
            const searchResponse = await axios_1.default.get(`${LUTRIS_API_URL}/games`, {
                params: { search: query },
                timeout: 10000,
            });
            ...
            const exactMatch = searchResponse.data.results.find(r => r.provider_games?.some(p => p.service === 'gog' && p.slug === gogIdStr));
            if (exactMatch) {
 ...
 */
async function searchLutrisInstallers(gameTitle, gogId) {
    try {
        ...
        let matchedGameSlug = null;
        const gogIdStr = String(gogId);
        for (const result of searchResult.results) {
            // Check if this game has our GOG ID in provider_games
            const gogProvider = result.provider_games?.find(p => p.service === 'gog' && p.slug === gogIdStr);
            if (gogProvider) {
        ...
        const gameResponse = await axios_1.default.get(`${LUTRIS_API_URL}/games/${matchedGameSlug}`, {
            timeout: 10000,
        });
        ...
        const gogInstallers = allWineInstallers.filter(installer => {
            const isGogInstaller = (installer.gogid === gogId) ||
 ...
 */
async function getLutrisInstaller(gameSlug, installerSlug) {
    try {
        const response = await axios_1.default.get(`${LUTRIS_API_URL}/games/${gameSlug}`, {
            timeout: 10000,
        });
 ...
 */
function parseLutrisScript(script) {
    const result = {
        requiresUserFile: false,
        userFilePrompt: undefined,
        wineArch: (script.game?.arch || 'win64'),
 ...
 */
function getInstallerFileKey(script) {
    if (!script.files)
        return null;
    for (const fileEntry of script.files) {
