...
// POST /api/online-sources/gog/exchange-code - Exchange authorization code for tokens
export async function POST(request: NextRequest) {
  try {
    ...
    console.error('Failed to exchange GOG code:', error);
    return NextResponse.json(
      { success: false, error: 'Failed to exchange code', message: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    );
  }
}
