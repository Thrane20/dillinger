...
import type { GamePlatformConfig } from '../types/game.js';
import { deriveWinePhase } from './wine-phase.js';

const createWinePlatform = (overrides: Partial<GamePlatformConfig> = {}): GamePlatformConfig => ({
...
test('returns needs_install when installation is not installed', () => {
  const phase = deriveWinePhase(
    { slug: 'test-game', title: 'Test Game' },
    createWinePlatform({ installation: { status: 'not_installed' } })
  );
  assert.equal(phase, 'needs_install');
});
...
test('returns installing when installation is running', () => {
  const phase = deriveWinePhase(
    { slug: 'test-game', title: 'Test Game' },
    createWinePlatform({ installation: { status: 'installing' } })
  );
...
test('returns install_failed when installation failed', () => {
  const phase = deriveWinePhase(
    { slug: 'test-game', title: 'Test Game' },
    createWinePlatform({ installation: { status: 'failed' } })
  );
