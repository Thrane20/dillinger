...
router.get('/', async (_req: Request, res: Response) => {
  try {
    const games = await storage.list('games');
    res.json({
      success: true,
      data: games,
      count: games.length,
...
router.get('/:id', async (req: Request, res: Response) => {
  try {
    const game = await storage.read('games', req.params.id);
...
router.post('/', createGameValidation, async (req: Request, res: Response) => {
  try {
    ...
    // Save to storage
    await storage.write('games', game.id, game);
...
router.put('/:id', createGameValidation, async (req: Request, res: Response) => {
  try {
    ...
    const existingGame = await storage.read('games', req.params.id);
    if (!existingGame) {
      return res.status(404).json({
        success: false,
        error: 'Game not found',
    ...
    await storage.write('games', req.params.id, updatedGame);
...
router.delete('/:id', async (req: Request, res: Response) => {
  try {
    ...
    await storage.delete('games', req.params.id);
